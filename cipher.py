# Name:Wanzhen Li
# Collabrator: Mingcheng (for def rotate num, how to start)
...
class Cipher:
    ...
    def encode_caesar(self, message, n):
        ...
        for letter in message:
            shifted_letter = self.rotate_letter(letter, n)
    ...
    def score_string(self, string):
        ...
        for letter in string:
            score += self.letter_heuristics[letter]
        return score
    ...
    def crack_caesar(self, cipher_text):
        ...
        cracked = ''
        for n in range(27):
            decoded_message = self.decode_caesar(cipher_text, n)
            score = self.score_string(decoded_message)
            if score > max_score:
                max_score = score
                cracked = decoded_message
        return cracked
    ...
    def encode_vigenere(self, message, key):
        ...
        for i in range(len(message)):
            ...
            rotation_number = self.letter_dict[key_letter]
            letter = message[i]
            shifted_letter = self.rotate_letter(letter, rotation_number)
            cipher_text += shifted_letter
        return cipher_text
